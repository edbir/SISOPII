...
struct ClientConnection
{
    ...
    ClientConnection(ClientConnection &&other) noexcept
        : network(other.network), syncManager(other.syncManager), clientThread(other.clientThread), isActive(other.isActive.load()), connectionId(std::move(other.connectionId))
    {
...
public:
    Server(int port, bool isPrimary)
        : port(port), isPrimary(isPrimary), fileManager("server_data")
    {
        ...
        if (isPrimary)
        {
            // Define os endereços dos backups (ajustar caso o Prof peça)
            backupAddresses.push_back({"127.0.0.1", 6001});
            backupAddresses.push_back({"127.0.0.1", 6002});

            for (const auto &[ip, port] : backupAddresses)
            {
                int sock = socket(AF_INET, SOCK_STREAM, 0);
                if (sock < 0)
                {
                    std::cerr << "[ERRO] Falha ao criar socket para backup.\n";
                    continue;
                }
                ...
                {
                    std::cerr << "[ERRO] Não foi possível conectar ao backup " << ip << ":" << port << "\n";
                    close(sock);
                    continue;
                }

                std::cout << "[INFO] Conectado ao backup " << ip << ":" << port << "\n";
                backupSockets.push_back(sock);
            }
        }
    }
